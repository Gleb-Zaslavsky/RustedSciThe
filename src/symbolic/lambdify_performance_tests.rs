...
mod performance_tests {
    ...
    fn create_complex_expression() -> Expr {
        ...
        Expr::cos(Box::new(x.clone().pow(Expr::Const(2.0)) + y.clone().pow(Expr::Const(2.0)))) +
    ...
    fn create_very_complex_expression() -> Expr {
        ...
        let part3 = Expr::arctg(Box::new(x.clone().pow(Expr::Const(3.0)) / (y.clone() + Expr::Const(1.0))));
    ...
    fn performance_comparison_lambdify_methods() {
        ...
        let expressions = vec![
            ("Simple", Expr::Var("x".to_string()) * Expr::Const(2.0) + Expr::Const(1.0)),
        ...
        for (name, expr) in expressions {
            ...
            println!("Compilation time - lambdify1: {:?}, lambdify2: {:?}", compile_time1, compile_time2);
            ...
            println!("Execution time ({} iterations) - lambdify1: {:?}, lambdify2: {:?}", iterations, exec_time1, exec_time2);
            ...
            assert!((result1 - result2).abs() < 1e-10, "Results differ: {} vs {}", result1, result2);
            ...
            println!("Performance ratios (lambdify1/lambdify2) - Compile: {:.2}x, Execute: {:.2}x", compile_ratio, exec_ratio);
        }
    }
    ...
    fn benchmark_compilation_overhead() {
        ...
        println!("lambdify1: {:?} (avg: {:?})", total_time1, total_time1 / compilations);
        println!("lambdify2: {:?} (avg: {:?})", total_time2, total_time2 / compilations);
        
        let ratio = total_time1.as_nanos() as f64 / total_time2.as_nanos() as f64;
        println!("Compilation speed ratio (lambdify1/lambdify2): {:.2}x", ratio);
    }
    ...
    fn benchmark_execution_patterns() {
        ...
        for (pattern_name, args) in patterns {
            ...
            println!("Results: {:.6} vs {:.6} (diff: {:.2e})", result1, result2, (result1 - result2).abs());
    ...
    fn performance_test_lambdify1_vs_lambdify2() {
        ...
        println!("  lambdify1: {:?} ({:.2} ns/call)", exec_time1, exec_time1.as_nanos() as f64 / iterations as f64);
        println!("  lambdify2: {:?} ({:.2} ns/call)", exec_time2, exec_time2.as_nanos() as f64 / iterations as f64);
        println!("  Ratio (1/2): {:.2}x\n", exec_time1.as_nanos() as f64 / exec_time2.as_nanos() as f64);
        
        // Verify results are identical
        let result1 = func1(&test_args);
        let result2 = func2(&test_args);
        assert!((result1 - result2).abs() < 1e-12, "Results differ: {} vs {}", result1, result2);
        println!("Result verification: {:.6} (both methods identical)", result1);
        ...
        println!("  Overall ratio (1/2): {:.2}x", total_time1.as_nanos() as f64 / total_time2.as_nanos() as f64);
    }
