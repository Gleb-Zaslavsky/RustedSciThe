//! Low-level packed storage for symbolic expressions.
//!
//! This module is where the crate earns most of its performance. Expressions are encoded
//! as compact byte slices with tiny tags and inline payloads, and higher layers operate on
//! borrowed views into those bytes instead of pointer-linked heap nodes. That design keeps
//! cache locality high, makes cloning cheap when it is needed, and lets iterators walk the
//! expression without allocating.
//!
//! The public types here come in owned/view pairs such as [`Num`] and [`NumView`],
//! [`Mul`] and [`MulView`]. Owned values manage a backing byte buffer, while views borrow
//! directly from existing packed storage.

//! Low-level representation of expressions.

use byteorder::{LittleEndian, WriteBytesExt};
use bytes::{Buf, BufMut};
use std::{borrow::Borrow, cmp::Ordering, hash::Hash, io::Read};

use crate::symbolic::View::coefficient::{Coefficient, CoefficientView};

use super::{
    Atom, AtomView, SliceType,
...
/// Borrowed raw byte slice containing a packed atom.
pub type BorrowedRawAtom = [u8];
/// A raw atom that does not have explicit variant information.
/// Owned byte buffer containing a packed atom.
...
/// Marker trait for types that can be used as packed-atom map keys.
...
/// An inline variable.
/// Stack-allocated packed variable used for cheap temporary arguments.
#[derive(Copy, Clone)]
...
/// Stack-allocated packed rational number used for cheap temporary arguments.
#[derive(Copy, Clone)]
...
/// Borrowed view of a packed variable atom.
...
/// A view of a [Fun].
#[derive(Debug, Copy, Clone, Eq, Hash)]
/// Borrowed view of a packed function atom.
...
/// A view of a [Num].
#[derive(Debug, Copy, Clone, Eq, Hash)]
/// Borrowed view of a packed numeric atom.
...
/// A view of a [Pow].
#[derive(Debug, Copy, Clone, Eq, Hash)]
/// Borrowed view of a packed power atom.
...
/// A view of a [Mul].
#[derive(Debug, Copy, Clone, Eq, Hash)]
/// Borrowed view of a packed product atom.
...
/// Iterator over child atoms in packed list storage.
...
/// Borrowed slice abstraction over packed child ranges.
