...
//! - banded Jacobian values chunk planning,
...
use crate::symbolic::codegen::codegen_tasks::{
    ...
    SparseExprEntry, SparseJacobianTask,
};
...
pub struct ResidualRuntimePlan<'a> {
    ...
    // chunks are guaranteed to be non-overlapping and cover the full output range, but may be
    // 1 or many depending on the chunking strategy and workload
...
impl<'a> ResidualTask<'a> {
    ...
    /// ResidualChunkPlan{ fn_name, output_offset, residuals, plan } for each chunk, where:
    /// - `fn_name` is a stable generated function name for the chunk.
    /// - `output_offset` is the global offset of the first residual entry written by this chunk.
    /// - `residuals` is the residual expressions evaluated by this chunk.
    /// - `plan` is the flattened low-level codegen plan for the chunk.
        ...
        let chunks = self
            ...
            .map(|(chunk_index, residuals)| {
                ...
                let fn_name = if chunk_index == 0 && residuals.len() == self.residuals.len() {
                    // case where the chunking strategy did not actually split the outputs, so
                    //we can keep the original function name
                    self.fn_name.to_string()
                } else {
                    format!("{}_chunk_{}", self.fn_name, chunk_index)
                };
                // residials Expressions for thus chunk packed into ResidualTask
                ...
                // Codegen plan for the chunk, which will be used to generate code for this chunk
...
/// One banded Jacobian values chunk in diagonal-major runtime order.
#[derive(Debug, Clone)]
pub struct BandedJacobianValuesChunkPlan<'a> {
    /// Stable generated function name for the chunk.
    pub fn_name: String,
    /// Global offset of the first value written by this chunk.
    pub value_offset: usize,
    /// Banded entries covered by this chunk in diagonal-major order.
...
impl<'a> BandedJacobianValuesChunkPlan<'a> {
    /// Returns the global half-open value range written by this chunk.
    pub fn value_range(&self) -> std::ops::Range<usize> {
        self.value_offset..(self.value_offset + self.entries.len())
    }
...
pub struct BandedJacobianStructure {
    ...
    pub kl: usize,
    pub ku: usize,
    pub diagonal_offsets: Vec<isize>,
...
impl BandedJacobianStructure {
    /// Returns the number of explicitly stored banded values.
    pub fn nnz(&self) -> usize {
...
/// Solver-facing banded Jacobian runtime plan.
///
/// The solver still sees one logical Jacobian provider:
/// - low-level generated chunks write banded values into disjoint slices,
/// - the wrapper owns the native band structure and can assemble
///   `BandedAssembly` on demand.
#[derive(Debug, Clone)]
pub struct BandedJacobianRuntimePlan<'a> {
    pub fn_name: &'a str,
    pub input_names: Vec<&'a str>,
...
impl<'a> BandedJacobianRuntimePlan<'a> {
    /// Returns the number of explicitly stored banded values.
    pub fn nnz(&self) -> usize {
        self.structure.nnz()
    }
...
impl<'a> BandedJacobianTask<'a> {
    /// Builds a solver-facing banded Jacobian runtime plan.
    pub fn runtime_plan(&self, strategy: BandedChunkingStrategy) -> BandedJacobianRuntimePlan<'a> {
        let chunks: Vec<BandedJacobianValuesChunkPlan<'a>> = self
            .chunk_with_strategy(strategy)
            .into_iter()
            .map(|chunk| BandedJacobianValuesChunkPlan {
                fn_name: chunk.chunk_fn_name(),
        ...
        BandedJacobianRuntimePlan {
            ...
            structure: BandedJacobianStructure {
                ...
                kl: self.kl,
                ku: self.ku,
                diagonal_offsets: self.entries.iter().map(|entry| entry.diag_offset).collect(),
                diagonal_positions: self
                    .entries
                    .iter()
...
/// Chooses a practical diagonal-based banded chunking strategy from the
/// currently available parallelism.
pub fn recommended_banded_chunking_for_parallelism(
    total_diagonals: usize,
    chunks_per_worker: usize,
) -> BandedChunkingStrategy {
    assert!(total_diagonals > 0, "total_diagonals must be positive");
    assert!(chunks_per_worker > 0, "chunks_per_worker must be positive");
    ...
    let workers = std::thread::available_parallelism()
        ...
        .unwrap_or(1);
    let target_chunks = (workers * chunks_per_worker).max(1);
    let diagonals_per_chunk = total_diagonals.div_ceil(target_chunks).max(1);
...
/// Chooses a conservative row-based sparse chunking strategy for `Auto`
/// execution policy.
///
/// Unlike the benchmarking-oriented helper that accepts `chunks_per_worker`,
/// this variant deliberately targets roughly one chunk per worker so compiled
/// fixtures do not start from an over-fragmented runtime plan.
pub fn recommended_row_chunking_for_auto_parallelism(total_rows: usize) -> SparseChunkingStrategy {
    assert!(total_rows > 0, "total_rows must be positive");

    let workers = rayon_overhead_baseline().workers.max(1);
    let rows_per_chunk = total_rows.div_ceil(workers).max(1);
    SparseChunkingStrategy::ByRowCount { rows_per_chunk }
}
...
/// Chooses a conservative residual chunking strategy for `Auto` execution
/// policy.
///
/// The goal is to avoid many tiny chunks on medium-sized systems while still
/// leaving enough independent work for each worker thread.
pub fn recommended_residual_chunking_for_auto_parallelism(
    total_outputs: usize,
) -> ResidualChunkingStrategy {
    assert!(total_outputs > 0, "total_outputs must be positive");

    let workers = rayon_overhead_baseline().workers.max(1);
    let max_outputs_per_chunk = total_outputs.div_ceil(workers).max(1);
    ResidualChunkingStrategy::ByOutputCount {
        max_outputs_per_chunk,
    }
}
...
/// Chooses a conservative dense-Jacobian row chunking strategy for `Auto`
/// execution policy.
pub fn recommended_dense_jacobian_chunking_for_auto_parallelism(
    total_rows: usize,
) -> DenseJacobianChunkingStrategy {
    assert!(total_rows > 0, "total_rows must be positive");
...
mod tests {
    ...
    #[test]
    fn banded_runtime_plan_assembles_native_banded_storage() {
        let e0 = Expr::Const(10.0);
        let e1 = Expr::Const(20.0);
        let e2 = Expr::Const(30.0);
        let entries = vec![
            BandedExprEntry {
                row: 1,
                col: 0,
        ...
        let asm = runtime_plan.assemble_banded_assembly(&[10.0, 20.0, 30.0]);

        assert_eq!(runtime_plan.nnz(), 3);
        assert_eq!(runtime_plan.chunks.len(), 3);
    ...
    #[test]
    fn recommended_banded_chunking_for_parallelism_returns_diagonal_strategy() {
        let strategy = recommended_banded_chunking_for_parallelism(17, 3);

        match strategy {
            BandedChunkingStrategy::ByDiagonalCount {
                diagonals_per_chunk,
            } => {
                assert!(diagonals_per_chunk >= 1);
                assert!(diagonals_per_chunk <= 17);
            }
            _ => panic!("expected diagonal-count chunking strategy"),
        }
    }
