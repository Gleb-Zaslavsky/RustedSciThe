...
pub struct AtomGeneratedBlockBreakdown {
    pub lower_many_ms: f64,
    pub peephole_ms: f64,
    pub reuse_temps_ms: f64,
    pub reuse_temps_applied: bool,
...
pub enum AtomTempReusePolicy {
    Always,
    Never,
    Auto,
}

impl Default for AtomTempReusePolicy {
    fn default() -> Self {
        Self::Auto
    }
}
...
impl GeneratedBlock {
    ...
    /// Builds a multi-output code generation block directly from packed
    /// `AtomView` expressions.
    pub fn from_atom_views(
        ...
        let symbols = vars
            ...
            .collect::<Vec<_>>();
        let vars_owned = vars.iter().map(|s| s.to_string()).collect::<Vec<_>>();
        Self::from_atom_views_with_symbols(fn_name, views, &vars_owned, &symbols, layout)
    }

    /// Builds a multi-output block directly from packed `AtomView`
    /// expressions using already prepared solver-order symbols.
    pub fn from_atom_views_with_symbols(
        fn_name: impl Into<String>,
        views: &[AtomView<'_>],
        vars: &[String],
        symbols: &[Symbol],
        ...
        Self::from_atom_views_with_symbols_with_breakdown(fn_name, views, vars, symbols, layout).0
    }

    /// Builds a multi-output block directly from packed `AtomView`
    /// expressions and reports how much time is spent in each lowering pass.
    pub fn from_atom_views_with_symbols_with_breakdown(
        fn_name: impl Into<String>,
        views: &[AtomView<'_>],
        vars: &[String],
        symbols: &[Symbol],
        ...
        Self::from_atom_views_with_symbols_with_breakdown_and_reuse_policy(
            fn_name,
            views,
            vars,
            symbols,
    ...
    pub fn from_atom_views_with_symbols_with_breakdown_and_reuse_policy(
        fn_name: impl Into<String>,
        views: &[AtomView<'_>],
        vars: &[String],
        symbols: &[Symbol],
        ...
        let reuse_begin = std::time::Instant::now();
        let final_ir = if should_reuse {
            optimized.reuse_temps()
        } else {
            optimized.clone()
        };
        let reuse_temps_ms = reuse_begin.elapsed().as_secs_f64() * 1_000.0;

        let breakdown = AtomGeneratedBlockBreakdown {
            lower_many_ms,
            peephole_ms,
            reuse_temps_ms,
            reuse_temps_applied: should_reuse,
            instruction_count: final_ir.instructions.len(),
            temp_count: final_ir.num_temps,
            output_count: final_ir.outputs.len(),
        };
...
fn should_apply_atom_temp_reuse(block: &LinearBlock, layout: Option<CodegenOutputLayout>) -> bool {
    match layout {
        ...
        Some(CodegenOutputLayout::Vector { .. }) => false,
        Some(CodegenOutputLayout::Matrix { .. }) | None => {
            if block.instructions.len() < 256 || block.outputs.len() <= 1 {
                return false;
            }
            let temp_pressure = block.num_temps.saturating_sub(block.outputs.len());
            temp_pressure >= 128
        }
    }
}
...
pub struct CodegenModule {
    ...
    /// Target language for code generation (defaults to Rust).
    language: Option<CodegenLanguage>,
}
...
impl CodegenModule {
    ...
    pub fn with_language(mut self, language: CodegenLanguage) -> Self {
        self.language = Some(language);
        self
    }

    /// Sets the target language for code generation (mutable version).
    pub fn set_language(&mut self, language: CodegenLanguage) {
        self.language = Some(language);
    }

    /// Returns the current target language (defaults to Rust).
    pub fn language(&self) -> CodegenLanguage {
        self.language.unwrap_or(CodegenLanguage::Rust)
    }
    ...
    /// Pushes a multi-output block lowered directly from packed atom views.
    pub fn push_atom_block(
        &mut self,
        fn_name: impl Into<String>,
        views: &[AtomView<'_>],
        vars: &[&str],
        ...
        self.blocks.push(GeneratedBlock::from_atom_views(
            fn_name, views, vars, layout,
        ));
    }

    /// Pushes an already prepared generated block.
    pub fn push_generated_block(&mut self, block: GeneratedBlock) {
        self.blocks.push(block);
    }

    pub fn emit_source(&self) -> String {
        match self.language() {
            CodegenLanguage::Rust => self.emit_rust_source(),
            CodegenLanguage::C => self.emit_c_source(),
            CodegenLanguage::Zig => self.emit_zig_source(),
        }
    }
    ...
    fn emit_zig_source(&self) -> String {
        ...
        for func in &self.functions {
            out.push_str(&ZigEmitter::emit_function(
                &func.ir,
                &func.fn_name,
                func.vars.len(),
            ));
        ...
        for block in &self.blocks {
            ...
            let emitted = match block.layout {
                Some(CodegenOutputLayout::Vector { len }) => {
                    ZigEmitter::emit_residual_block_function(
                        &block.ir,
                        &block.fn_name,
                        block.vars.len(),
                        len,
    ...
    /// Emits a C header file with function declarations.
    pub fn emit_c_header(&self) -> String {
        ...
        for func in &self.functions {
            function_names.push((func.fn_name.clone(), false));
        }

        for block in &self.blocks {
            function_names.push((block.fn_name.clone(), true));
        }

        CEmitter::emit_header(&function_names)
    }
    ...
    pub fn total_block_instruction_count(&self) -> usize {
        self.blocks
            .iter()
            .map(GeneratedBlock::instruction_count)
            .sum()
    }

    pub fn total_block_temp_count(&self) -> usize {
        self.blocks.iter().map(GeneratedBlock::temp_count).sum()
    }

    pub fn max_block_instruction_count(&self) -> usize {
        self.blocks
            .iter()
            .map(GeneratedBlock::instruction_count)
            .max()
    ...
    pub fn total_block_output_count(&self) -> usize {
        self.blocks.iter().map(GeneratedBlock::output_count).sum()
    }
...
mod tests {
    use super::*;
    use crate::symbolic::codegen::codegen_generated_fixtures::{
