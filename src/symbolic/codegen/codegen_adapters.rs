...
use crate::symbolic::codegen::CodegenIR::{CodegenModule, GeneratedBlock, LinearBlock};
use crate::symbolic::codegen::codegen_provider_api::{
    BackendKind, MatrixBackend, PreparedBandedProblem, PreparedDenseProblem, PreparedSparseProblem,
};
use crate::symbolic::codegen::codegen_runtime_api::{
    BandedJacobianRuntimePlan, BandedJacobianValuesChunkPlan, DenseJacobianChunkPlan,
    DenseJacobianChunkingStrategy, DenseJacobianRuntimePlan, ResidualChunkPlan,
...
use crate::symbolic::codegen::codegen_tasks::{
    BandedChunkingStrategy, BandedJacobianTask, IvpJacobianTask, IvpResidualTask, JacobianTask,
    ResidualTask, SparseChunkingStrategy, SparseJacobianTask,
};
...
pub fn generated_block_from_banded_chunk(
    chunk: &BandedJacobianValuesChunkPlan<'_>,
) -> GeneratedBlock {
    GeneratedBlock::from_sparse_values_plan(&chunk.plan)
}
...
pub fn prepared_banded_problem<'a>(
    backend_kind: BackendKind,
    matrix_backend: MatrixBackend,
    residual_task: &ResidualTask<'a>,
    residual_strategy: ResidualChunkingStrategy,
    jacobian_task: &BandedJacobianTask<'a>,
    jacobian_strategy: BandedChunkingStrategy,
) -> PreparedBandedProblem<'a> {
    PreparedBandedProblem::new(
        backend_kind,
        matrix_backend,
        residual_runtime_plan(residual_task, residual_strategy),
        jacobian_task.runtime_plan(jacobian_strategy),
    )
}
...
pub fn banded_ir_blocks(plan: &BandedJacobianRuntimePlan<'_>) -> Vec<(usize, LinearBlock)> {
    plan.chunks
        .iter()
        .map(|chunk| {
            let block = generated_block_from_banded_chunk(chunk);
            (chunk.value_offset, block.ir)
        })
        .collect()
}
...
pub fn banded_module(module_name: &str, plan: &BandedJacobianRuntimePlan<'_>) -> CodegenModule {
    let mut module = CodegenModule::new(module_name);
    for chunk in &plan.chunks {
        module.push_sparse_values_plan(&chunk.plan);
    }
...
mod tests {
    ...
    use crate::symbolic::codegen::codegen_tasks::SparseExprEntry;
