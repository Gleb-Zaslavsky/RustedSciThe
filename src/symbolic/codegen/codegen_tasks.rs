...
pub struct CodegenTaskPlan<'a> {
    ...
    // residual, jacobian, ivp residual, ivp jacobian, sparse jacobian values
...
//========================================================================================
// the result of planning stage for various codegen tasks, which is a flattened description
//of what to generate and how to call it
// Code provides via CodegenTask trait functions for every task type,
// and a plan() method that produces a CodegenTaskPlan with flattened inputs and outputs
// ready for the lowerer/emitter pipeline
...
impl<'a> ResidualTask<'a> {
    ...
    pub fn plan(&self) -> CodegenTaskPlan<'a> {
        CodegenTaskPlan {
            ...
            //  flat list of parameters adn variables in the default order for future codegen backends
            input_names: self.flattened_argument_names(),
            // Vec of PlannedOutput{ expr, coordinate: None} since residuals are just flat vectors without coordinate metadata
            // vector of output expressions with no coordinate metadata, since residuals are just flat vectors
            outputs: self
                .residuals
                .iter()
...
impl<'a> JacobianTask<'a> {
    ...
    pub fn plan(&self) -> CodegenTaskPlan<'a> {
        ...
        // outputs is a flat vector of PlannedOutput with coordinate metadata for each entry,
        // since Jacobians are matrices and we need to know the row and column of each entry
...
/// One explicitly stored symbolic Jacobian entry tagged with band metadata.
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct BandedExprEntry<'a> {
    pub row: usize,
    pub col: usize,
    pub diag_offset: isize,
    pub diag_position: usize,
    pub expr: &'a Expr,
}
...
/// Practical chunking strategies for banded Jacobian values codegen.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BandedChunkingStrategy {
    /// Keep the full banded values task as one block.
    Whole,
    /// Split diagonal ranges into a target number of coarse chunks.
    ByTargetChunkCount { target_chunks: usize },
    /// Split by a maximum number of diagonals per chunk.
    ByDiagonalCount { diagonals_per_chunk: usize },
}
...
/// Banded Jacobian task for native banded runtime assembly.
#[derive(Debug, Clone)]
pub struct BandedJacobianTask<'a> {
    pub fn_name: &'a str,
    pub shape: (usize, usize),
    pub kl: usize,
    pub ku: usize,
...
/// One chunk of a banded Jacobian values task.
#[derive(Debug, Clone)]
pub struct BandedJacobianChunkTask<'a> {
    pub base_fn_name: &'a str,
    pub chunk_index: usize,
...
impl<'a> CodegenTask<'a> for BandedJacobianTask<'a> {
    fn fn_name(&self) -> &'a str {
        self.fn_name
    }
...
impl<'a> BandedJacobianTask<'a> {
    /// Builds a flattened banded-Jacobian-values plan in diagonal-major order.
    pub fn plan(&self) -> CodegenTaskPlan<'a> {
        CodegenTaskPlan {
            fn_name: Cow::Borrowed(self.fn_name()),
    ...
    /// Splits banded entries by contiguous diagonal groups.
    pub fn chunk_with_strategy(
        &self,
        strategy: BandedChunkingStrategy,
    ) -> Vec<BandedJacobianChunkTask<'a>> {
        let total_diagonals = self.kl + self.ku + 1;
        ...
        let mut chunk_entries: Vec<Vec<BandedExprEntry<'a>>> = Vec::new();
        let mut chunk_offsets: Vec<usize> = Vec::new();
        let mut chunk_diag_ranges: Vec<Option<(isize, isize)>> = Vec::new();

        for (entry_index, entry) in self.entries.iter().copied().enumerate() {
            let bucket = ((entry.diag_offset + self.kl as isize) as usize) / diagonals_per_chunk;
            if bucket >= chunk_entries.len() {
                chunk_entries.resize_with(bucket + 1, Vec::new);
                chunk_offsets.resize(bucket + 1, 0);
                chunk_diag_ranges.resize(bucket + 1, None);
            }
            if chunk_entries[bucket].is_empty() {
                chunk_offsets[bucket] = entry_index;
            }
            chunk_entries[bucket].push(entry);
            chunk_diag_ranges[bucket] = Some(match chunk_diag_ranges[bucket] {
                Some((start, _)) => (start, entry.diag_offset),
                None => (entry.diag_offset, entry.diag_offset),
            });
        }
        ...
        chunk_entries
            ...
            .map(|(chunk_index, entries)| {
                let (diag_start, diag_end) = chunk_diag_ranges[chunk_index]
                    .expect("non-empty banded chunk must have a diagonal range");
                BandedJacobianChunkTask {
                    base_fn_name: self.fn_name,
                    chunk_index,
...
impl<'a> CodegenTask<'a> for BandedJacobianChunkTask<'a> {
    fn fn_name(&self) -> &'a str {
        self.base_fn_name
    }
...
impl<'a> BandedJacobianChunkTask<'a> {
    /// Returns a stable generated function name for this chunk.
    pub fn chunk_fn_name(&self) -> String {
        format!("{}_chunk_{}", self.base_fn_name, self.chunk_index)
    }

    /// Builds a flattened banded-values plan for this chunk.
    pub fn plan(&self) -> CodegenTaskPlan<'a> {
        CodegenTaskPlan {
            fn_name: Cow::Owned(self.chunk_fn_name()),
            kind: CodegenTaskKind::BandedJacobianValues,
...
mod tests {
    ...
    #[test]
    fn banded_jacobian_plan_keeps_banded_kind_and_coordinates() {
        let e0 = Expr::Var("y0".to_string());
        let e1 = Expr::Var("y1".to_string());
        let entries = vec![
            BandedExprEntry {
    ...
    #[test]
    fn banded_chunking_strategy_by_diagonal_count_groups_diagonals() {
        let e0 = Expr::Var("l".to_string());
        let e1 = Expr::Var("d0".to_string());
        let e2 = Expr::Var("u".to_string());
        let e3 = Expr::Var("d1".to_string());
        let entries = vec![
            BandedExprEntry {
                row: 1,
                col: 0,
        ...
        assert_eq!(chunks[0].diagonal_range, -1..=-1);
        assert_eq!(chunks[1].value_offset, 1);
        assert_eq!(chunks[1].diagonal_range, 0..=0);
        assert_eq!(chunks[1].entries.len(), 2);
        // Banded chunk offsets are expressed in the original flattened
        // values order, not as a post-compaction offset after previous
        // chunks. The upper diagonal first appears at entry index 2.
        assert_eq!(chunks[2].value_offset, 2);
        assert_eq!(chunks[2].diagonal_range, 1..=1);
    }
}
