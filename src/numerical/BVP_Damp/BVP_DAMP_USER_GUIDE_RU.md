...
`new(...)` constructor is retained for compatibility, but new code should prefer presets plus builder methods. In particular, prefer typed derivative-scheme builders over raw `scheme: String`.
...
Схема дискретизации выбирается через typed builder (`forward_derivative`, `trapezoid_derivative`, `with_scheme`). `strategy` обычно `"Damped"`. `linear_sys_method` лучше оставлять `None`, если вы выбираете Sparse/Banded через `method` и generated backend config. `method` - это крупный выбор матричного пути: `"Dense"`, `"Sparse"` или `"Banded"`.
...
Typed API для схемы дискретизации доступен и для Damped, и для Frozen:
...
`with_scheme_name("...")` оставлен только как compatibility escape hatch для legacy или experimental scheme names.
...
### Lifecycle AOT-артефактов и cleanup

AOT-артефакты создаются внутри crate `target` и отслеживаются через
`AotResolver`: ключ задачи (`problem_key`) и manifest metadata. Это и есть
source of truth. Не редактируйте generated-папки вручную и не делайте вывод о
корректности артефакта только по имени файла. Если меняется сетка или
дискретизованная задача, symbolic frontend, matrix route, toolchain или
chunking policy, рассматривайте это как отдельный artifact lifecycle и
позвольте manifest/resolver contract решить, можно ли переиспользовать старый
артефакт.

Рекомендуемый lifecycle явный:

1. `BuildIfMissing` - setup/warmup-запуск, в котором разрешено создать
   артефакт.
2. `RequirePrebuilt` - измеряемый или production-цикл, где отсутствующий
   артефакт должен быть typed error, а не скрытая компиляция.
3. `cleanup_registered_aot_artifacts()` - явная очистка для
   story/debug/cold-build workflow. Вызывайте ее только после того, как compiled
   callbacks этого солвера больше не нужны.

```rust
use RustedSciThe::numerical::BVP_Damp::generated_solver_handoff::AotBuildPolicy;

// Первый проход: BuildIfMissing preset, например
// GeneratedBackendConfig::banded_atomview_build_if_missing_release_tcc().
solver.try_eq_generate()?;
solver.solve();

// Строгий repeated-solve проход: используем уже собранный артефакт
// без скрытой компиляции.
...
// Необязательный cleanup для story/debug/cold-build workflow.
// Не делайте это, пока нужны live callbacks этого solver.
let removed = solver.cleanup_registered_aot_artifacts()?;
println!("removed {removed} generated AOT artifact trees");
```

Cleanup намеренно не выполняется автоматически в `drop`. Compiled callbacks
могут еще жить в процессе, а на Windows загруженная dynamic library может
держать file handles. Метод чистит только registered generated trees из
текущего resolver snapshot; он не unregister linked callbacks и не unload
dynamic libraries. Это менее магично, зато безопаснее как production-контракт.
...
Самый прямой production-синтаксис - оставить chunking policy по умолчанию и
попросить compiled backend самому выбрать runtime execution:
...
После `solver.try_solver()?` смотрите не на то, что вы попросили в конфиге, а
на то, что реально попало в diagnostics:
...
Смысл разделения такой: `aot.auto.*` описывает план, а `aot.runtime.*` описывает
фактическое поведение уже связанного callback. В 12 Core story-тесте
`combustion-1000` для AtomView+tcc Auto выбрал `Sequential` и для Sparse, и для
Banded. План объяснил это как `work_per_chunk_too_small` для стадии Jacobian и
`single_chunk_or_job` для residual; runtime diagnostics подтвердили это через
`actual_jobs = 1`, `parallel_requested = false` и fallback reason
`single_requested_job`. Это успешное решение Auto, а не отказ параллелизма.
Forced chunking имеет смысл включать только тогда, когда большая модель или
break-even тест показывает достаточно callback work per job, чтобы окупить
