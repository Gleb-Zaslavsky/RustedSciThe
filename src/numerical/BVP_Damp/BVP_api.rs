...
impl BVP {
    pub fn new(
        ...
        if strategy=="Frozen"||strategy=="Naive" {
            ...
            let nrbvp =  NRBVP::new(eq_system.clone(),
                initial_guess.clone(), 
                values.clone(), 
                arg.clone(),
                BorderConditions.clone(), t0, t_end, n_steps,strategy.clone(), strategy_params.clone(), linear_sys_method.clone(), method.clone(), tolerance, max_iterations);
            let structure =Some(nrbvp);
            let structure_damp =None;
            (structure, structure_damp,rel_tolerance,Bounds )
        }
        else if strategy=="Damped"{
            let  nrbvpd =  NRBDVPd::new(eq_system.clone(),
                initial_guess.clone(), 
                values.clone(), 
                arg.clone(), 
     ...
     pub fn solve(&mut self) {
        if let Some(ref mut structure) = self.structure {   structure.solve(); }
        if let Some(ref mut structure_damp) = self.structure_damp { structure_damp.solve(); }
     }
     pub fn get_result(&self)  -> Option<DMatrix<f64>> 
     {
        if let Some( structure) = &self.structure {   let res = structure.get_result(); return res}
        if let Some( structure_damp) = &self.structure_damp { let res = structure_damp.get_result(); return res }
