...
pub fn twpnt_refinement(y_DMatrix: DMatrix<f64>, x_mesh: DVector<f64>,  d: f64, g: f64, C:f64, abs_tol: f64,) -> (Vec<f64>, DMatrix<f64>, usize) {
...
impl Grid {
    fn new(y_DMatrix: DMatrix<f64>, x_mesh: DVector<f64>,  d: f64, g: f64, C:f64, abs_tol: f64,) -> Self {
    ...
    fn assert_monotonically_growing(vector: &[f64]) {
        for window in vector.windows(2) {
            if !(window[0]<=  window[1]) { println!("  vector of length {} is non-monotonically growing{:?}, ", vector.len(), vector)}
            assert!(window[0] <= window[1], "Vector elements are not monotonously growing");
        }
    }   
     ...
     //  println!("y_DMatrix {}", y_DMatrix.clone());
        ...
        for (j, y_j) in y_DMatrix.row_iter().enumerate() {
            ...
            for i in  0..n- 1 {
                ...
                if dy_i.abs()  > delta {println!("dy_i/dalta: {}",  round_to_n_digits(dy_i.abs()/delta, 3 ))};
                list_dy_i.push(dy_i);
            ...
            for i in 1..n - 1 {
                ...
                if differ_dy > gamma { println!("differ_dy/gamma = {}", round_to_n_digits(differ_dy/gamma , 3)) };
                if 0.0 < derivative_range_i {
                    let relative_dy_dx_i = f64::max(relative_dy_dx[i], differ_dy / derivative_range_i);
        ...
        info!("\n \n difference grid condition violated in {} intervals", Self::count_non_zero_elements(&vary_y));
        info!("gradient grid condition violated in {} intervals", Self::count_non_zero_elements(&vary_dy_dx));
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        // the weight assigned to each interval, which is determined by the number of times the solution variables' variations exceed certain thresholds.
            ...
            for i in 0..self.n - 1 {
                ...
                if weights[i] >  0  {most +=  1}
            }
            info!("indexes of non-zero weights {:?} ", weights.iter().enumerate().filter(|(_, &x)| x != 0).map(|(i, _)| i).collect::<Vec<_>>() );
            info!("number of non-zero weights {} ", Self::count_non_zero_elements(&DVector::from_vec(weights.clone())));
            ...
            let least =
            if more>0  {sorted_weights[more] } //  If the number of intervals to halve is greater than zero, the least weight is set to the weight of the interval with the corresponding index
            else { 1 + sorted_weights[0] };// Otherwise, the least weight is set to one plus the weight of the first interval.
            ...
            info!("number of marked intervals {} ", Self::count_non_false_elements(&mark));
            self.mark = mark;
            self.more = more;
    ...
    fn add_points(&mut self) {
        ...
        if more > 0 {
           // let length: f64 = (x[n_former] - x[0]).abs();
            ...
            let error = counted > 0;
            if error { error!("error");} else { info!("no degenerated intervals");}
            ...
            for i_old in (1..n_former).rev() {
                ...
                if mark[i_old - 1] {
                    ...
                    info!("inserted new grid point {} between points {} and {} at index {} ", round_to_n_digits( new_grid[n_new - 1], 3), 
                    round_to_n_digits( new_grid[n_new ], 3), round_to_n_digits( new_grid[n_new - 2], 3), i_old - 1  );
            ...
            log::info!("created new grid {:?} of length {}",new_grid,  new_grid.len());
            log::info!(
           ...
           if self.more==0 {assert_eq!(new_initial_guess, self.y_DMatrix);
