...
impl NR {
    pub fn new() -> NR {  //jacobian: Jacobian, initial_guess: Vec<f64>, tolerance: f64, max_iterations: usize, max_error: f64, result: Option<Vec<f64>>
        NR {
    ...
    pub fn set_equation_sysytem(&mut self, jacobian: Jacobian, initial_guess: Vec<f64>, tolerance: f64, max_iterations: usize) {
        assert_eq!(jacobian.vector_of_variables.len(), initial_guess.len(), "Initial guess and vector of variables should have the same length.");
        ...
        assert!(!self.initial_guess.is_empty(), "Initial guess should not be empty.");
        assert!(tolerance >= 0.0, "Tolerance should be a non-negative number.");
        assert!(max_iterations > 0, "Max iterations should be a positive number.");
     
       
    }
    ///Set system of equations with vector of symbolic expressions
    pub fn eq_generate(&mut self, eq_system: Vec<Expr>, initial_guess: Vec<f64>, tolerance: f64, max_iterations: usize) {

        let mut args_: Vec<String> = eq_system.iter().map(|x| x.all_arguments_are_variables()).flatten().collect::<Vec<String>>();
        args_.sort();
        args_.dedup();
        let args: Vec<&str> = args_.iter().map(|x| x.as_str()).collect(); 
        assert!(!args.is_empty(), "No variables found in the equations."); 
        assert_eq!(args.len()==eq_system.len(), true, "Equation system and vector of variables should have the same length.");
        let mut Jacobian_instance = Jacobian::new();
    ...
    pub fn eq_generate_from_str(&mut self, eq_system_string: Vec<String>, initial_guess: Vec<f64>, tolerance: f64, max_iterations: usize) {
        ...
        assert!(!initial_guess.is_empty(), "Initial guess should not be empty.");
        assert_eq!(eq_system.len()==initial_guess.len(), true, "Equation system and initial guess should have the same length.");
    ...
    pub fn iteration(&mut self, x: Vec<f64>) -> Vec<f64> {
        ...
        assert!(!Jacobian_instance.evaluated_jacobian_DMatrix.is_empty(), "Jacobian should not be empty.");
        assert!(!Jacobian_instance.evaluated_functions_DVector.is_empty(), "Functions should not be empty.");
     ...
     //  let j_inverse = <Matrix<f64, Dyn, Dyn, VecStorage<f64, Dyn, Dyn>> as Clone>::clone(&new_j).try_inverse().unwrap();
        ...
        let   new_x = x.iter().zip(&dx).map(|(x_i, dx_i)| x_i - dx_i).collect::<Vec<f64>>(); 
...
pub fn solve(&mut self) -> Option<Vec<f64>> {
    ...
    while i < self.max_iterations  {
        ...
        let  dx = new_x.iter().zip(&x).map(|(x_i, x_j)| (x_i - x_j).abs()).collect::<Vec<f64>>();
    ...
    pub fn solve_linear_LU(coeffs:Vec<Vec<f64>>, constants: Vec<f64>) -> Result< Vec<f64>, &'static str> {
