...
impl<F, J> Ros2Solver<F, J>
where
    F: Fn(f64, &[f64], &mut [f64]),
    J: Fn(f64, &[f64], &mut Vec<Triplet<usize, usize, f64>>),
{
    pub fn new(f: F, jac: J, cfg: Ros2Config) -> Self {
        Self {
            f,
            jac,
            cfg,
    ...
    pub fn integrate(
        ...
        t_end: f64,
        h0: f64,
    ) -> Result<(f64, Vec<f64>), Ros2Error> {
        let n = y0.len();
        if n == 0 {
            return Err(Ros2Error::DimensionMismatch);
        }
        ...
        for _ in 0..self.cfg.max_steps {
            ...
            if step.err_norm <= 1.0 {
                ...
                let factor = step_size_factor(step.err_norm, 1.0, self.cfg.safety);
                h *= factor;
                ...
                if rejects_in_row > self.cfg.max_rejects {
                    return Err(Ros2Error::TooManyRejects);
                }
                ...
                if h.abs() < self.cfg.h_min {
                    return Err(Ros2Error::StepUnderflow);
                }
            }
        }

        Err(Ros2Error::TooManySteps)
    }

    fn try_step(&mut self, t: f64, y: &[f64], h: f64) -> Result<Ros2Step, Ros2Error> {
        let n = y.len();

        // KPP / Hairer-style ROS-2:
        //
        // gamma = 1 + 1 / sqrt(2)
        //
        // Original KPP form:
        ...
        //
        // We solve the equivalent scaled system:
        //
        //     A = I - h gamma J
        //     A k_i = h gamma f(T_i, Y_i)
        //             + gamma * sum_j c_ij k_j
        //
        // This avoids explicit 1 / h scaling in the sparse matrix.
        ...
        let e1 = 1.0 / (2.0 * gamma);
        let e2 = 1.0 / (2.0 * gamma);
        ...
        let lu = a.sp_lu().map_err(|_| Ros2Error::SingularMatrix)?;
        ...
        // Y2 = y + a21 k1
        //
        // A k2 = h gamma f(t + h, y + a21 k1)
        //        + gamma c21 k1
        let mut y_stage = vec![0.0; n];
        for i in 0..n {
            y_stage[i] = y[i] + a21 * k1[i];
        }

        let mut f_stage = vec![0.0; n];
        (self.f)(t + h, &y_stage, &mut f_stage);
        self.stats.rhs_evals += 1;
        check_finite_slice(&f_stage)?;

        let mut rhs2_vec = vec![0.0; n];
        for i in 0..n {
            rhs2_vec[i] = h * gamma * f_stage[i] + gamma * c21 * k1[i];
        }
...
fn check_finite_slice(x: &[f64]) -> Result<(), Ros2Error> {
    if x.iter().all(|v| v.is_finite()) {
        Ok(())
    } else {
        Err(Ros2Error::InvalidNumber)
    }
}
...
fn build_ros_matrix(
...
) -> Result<SparseColMat<usize, f64>, Ros2Error> {
    ...
    SparseColMat::<usize, f64>::try_new_from_triplets(n, n, &triplets)
        .map_err(|_| Ros2Error::DimensionMismatch)
}
...
mod tests {
    ...
    fn main_test() {
        ...
        let mut solver = Ros2Solver::new(f, jac, cfg);
...
mod correctness_tests {
    ...
    #[test]
    fn scalar_stiff_decay_matches_exact_solution_ros2_reasonable_tol() {
        let lambda = 1000.0;
