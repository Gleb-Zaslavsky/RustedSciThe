...
pub fn newton_tol(rtol: NumberOrVec)-> f64 {
    let newton_tol:f64 = match rtol {
        ...
        NumberOrVec::Number(rtol_i) => { 
            f64::max(10.0 * f64::EPSILON / rtol_i, f64::min(0.03, rtol_i.powf(0.5)))
         }
        NumberOrVec::Vec(rtol_i) => {
            let rtol_i = *rtol_i.iter().min_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal)).unwrap();
            f64::max(10.0 * f64::EPSILON / rtol_i, f64::min(0.03, rtol_i.powf(0.5)))
        }
...
fn check_if_is_dvector(obj: &dyn std::any::Any) -> bool {
    ...
    let fun_wrapped = Box::new(move |t: f64, y: &DVector<T>| -> DVector<T> {
        fun(t, y)
    });
...
pub fn scale_func(rtol: NumberOrVec, atol: NumberOrVec, y0: &DVector<f64>) -> Vec<f64> {
    ...
    match atol {
        ...
        NumberOrVec::Number(atol) => {
            match rtol {
                // rtol is a number
                NumberOrVec::Number(rtol) => {
                    y0.into_iter().map( |&y_i| atol + (y_i.abs() * rtol)).collect()
                     }
 ...
 //   println!("scale, {}, d2, {}, h1 {}", scale, d2.clone(), h1);
  //  println!("select initial step: done" );
   // println!("h0, {}, h1, {}, interval_length, {}, max_step, {}", h0, h1, interval_length, max_step);
    ...
    let res = vec![100.0*h0, h1, interval_length, max_step].into_iter().fold(1.0, |acc, x| acc.min(x));
...
pub fn validate_tol(rtol: NumberOrVec, atol: NumberOrVec, _n: usize) -> Result<(NumberOrVec, NumberOrVec), &'static str> {
    let rtol = match rtol {
        // rtol is a number
        NumberOrVec::Number(rtol_i) => {
...
) -> (DMatrix<f64>, DVector<f64>)
{
    ...
    fn ret(v: &Vec<f64>, indices: &Vec<usize>) -> Vec<f64> {
        indices.iter().filter_map(|&index| v.get(index)).cloned().collect()
    }
   ...
   let  y_new:DMatrix<f64> = DMatrix::from_fn(n, n, |i, j| y[j] + h[i]);
    ...
    fn create_f_diff(y_new:DMatrix<f64>,t:f64, fun:&Box<dyn Fn(f64, &DVector<f64>) -> DVector<f64>>, f:&DVector<f64> ) ->( DMatrix<f64>,  DMatrix<f64>){
    let n = y_new.nrows();
    ...
    let mut diff_vec = Vec::new();
       for row in y_new.row_iter() {
        ...
        let diff_vec_i: Vec<f64> =   fun_new_vec_i.clone().iter().zip( f.data.as_vec().to_owned() ).map(|(fi, f)| fi - f).collect();
...
for column in diff.column_iter() {  // iterate over each column
    let max_index = column
        .iter()
        .enumerate()
        .filter_map(|(index, &val)| if val == column.amax() { Some(index) } else { None })//finds the index of the maximum absolute difference using the enumerate() 
...
/* 
    ...
    let max_diff_vec: Vec<_> = r.iter().zip(&max_indices).map(|(r_i, max_ind)| diff[( *max_ind, *r_i)]).collect();
  ...
  //  let max_diff:Vec<f64> = max_diff.into();
    ...
    let f_new_max: Vec<_> = r.iter().zip(&max_indices).map(|(r_i, max_ind)| f_new[( *max_ind, *r_i)]).collect();
    ...
    let scale_vec:Vec<f64> = f_new_max.iter().zip(&f_max).map(|(fi, fi_max)| fi_max.abs().max(fi.abs())).collect();
    let scale = DMatrix::from_vec(n,1, scale_vec.clone());
    // element-wise compare
    let diff_too_small:Vec<bool> = max_diff.iter().zip(scale_vec.clone()).map(|( max_diff_i, scale_i)| *max_diff_i<NUM_JAC_DIFF_SMALL*(scale_i)   ).collect();
    ...
    if diff_too_small.iter().any(|&x| x) {
        ...
        let h_new = y.select_rows(&ind) 
        + new_factor.component_mul(&y_scale.select_rows(&ind)) - y.select_rows(&ind);
        ...
        for column in diff_new.column_iter() {  // iterate over each column
            let max_index = column
                .iter()
                .enumerate()
                .filter_map(|(index, &val)| if val == column.amax() { Some(index) } else { None })//finds the index of the maximum absolute difference using the enumerate() 
        ...
        let max_diff_new: Vec<_> = r.iter().zip(&max_indices).map(|(r_i, max_ind)| diff_new[(  *max_ind, *r_i)]).collect();
        let max_diff_new = DVector::from_vec(max_diff_new);
        let scale_new_vec:Vec<f64> = f_new_max.iter().zip(&f_max).map(|(fi, fi_max)| fi_max.abs().max(fi.abs())).collect();
        let scale_new = DMatrix::from_vec(n,1, scale_new_vec.clone());
        let max_diff_scale:Vec<f64> = ret(&max_diff_vec, &ind  ).iter().zip(&scale_new_vec).map(|( max_diff_i, scale_i)| *max_diff_i *(*scale_i)   ).collect();
     
        let max_diff_new_scale:Vec<f64> = max_diff_new.iter().zip(&ret(&scale_vec, &ind) ).map(| ( max_diff_i, scale_i)| *max_diff_i *(*scale_i) ).collect();

        let update:Vec<bool> = max_diff_scale.iter().zip(&max_diff_new_scale).map(| ( max_diff_i, max_diff_new_scale_i)| *max_diff_i < *max_diff_new_scale_i  ).collect();
        if update.iter().any(|&x| x) {
            ...
            factor.select_rows(&update_ind).copy_from(&new_factor.select_rows(&update_ind));
            h.select_rows(&update_ind).copy_from(&h_new.select_rows(&update_ind));
            diff.select_columns(&update_ind).copy_from(&diff_new.select_columns(&update_ind));
            scale.select_rows(&update_ind).copy_from(&scale_new.select_rows(&update_ind));
            max_diff.select_rows(&update_ind).copy_from(&max_diff_new.select_rows(&update_ind));
        }
    }
    //diff/=h 
