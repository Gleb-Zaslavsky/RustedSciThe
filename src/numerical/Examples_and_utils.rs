...
pub fn plot_from_expr(input: String, x_name: &str, y_name: &str, start: f64, end: f64, num_values: usize) {
        ...
        let x = (0..num_values).map(|i| start +  i as f64 * step).collect::<Vec<_>>();
       // println!("result = {:?} \n", y);
        plots(x_name.to_string(), vec![y_name.to_string()], DVector::from_vec(x)  , y_DMatrix);
}
...
impl NonlinEquation {
    ...
    pub fn exact_solution(&self, start: Option<f64>, end: Option<f64>,  num_values: Option<usize>) ->Vec<f64> {
        match self {
            NonlinEquation::LaneEmden5 => {
                let start = if let Some(start) = start {start} else {0.0};
                let end = if let Some(end) = end {end} else {1.0};  
                let num_values = if let Some(num_values) = num_values {num_values} else {100};
...
pub fn Bounds(&self) -> HashMap<String, (f64, f64)> {
    match self {
        ...
        NonlinEquation::Clairaut => {
            let Bounds = HashMap::from([  ("z".to_string(), (-10.0, 10.0),    ),
            ("zz".to_string(), (-10.0, 10.0),    ), ("y".to_string(), (-7.0, 7.0),    ) ]);
   
            Bounds
        }
        NonlinEquation::TwoPointBVP => {
            HashMap::from([  ("z".to_string(), (-1.0, 1.0),    ), ("y".to_string(), (1e-26, 1.0),    ) ])
        }
    }
}//Bounds
pub fn rel_tolerance(&self) ->HashMap<String, f64> {
    match self {
        ...
        NonlinEquation::Clairaut =>  {
            HashMap::from([  ("zz".to_string(), 1e-4    ),("z".to_string(), 1e-4    ), ("y".to_string(), 1e-4,    ) ])
        }
        NonlinEquation::TwoPointBVP => {
            HashMap::from([  ("z".to_string(), 1e-4    ), ("y".to_string(), 1e-4,    ) ])
        }
