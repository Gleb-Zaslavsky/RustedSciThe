...
impl Display for BE {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "BE {{ t0: {}, t_bound: {}, t: {}, y: {:?} }}", self.t0, self.t_bound, self.t, self.y)
    }
}
...
impl BE {
    pub fn new() -> BE {
        ...
        let nr_new = NRE::new(Vec::new(), DVector::zeros(0), Vec::new(), String::new(), 0.0, 0, 0.0, true, None);
        BE {
            newton: nr_new,
            y0: DVector::zeros(0),
            t0: 0.0,
    ...
    pub fn set_initial(
        ...
        let nr = if let Some(dt) = h {
            self.global_timestepping = true;
             NRE::new(eq_system, initial_guess, values, arg, tolerance, max_iterations,  dt, true, None)
             
        } else {
            println!("global_timestepping = false");
            self.global_timestepping = false;
            NRE::new(eq_system, initial_guess, values, arg, tolerance, max_iterations, 1e-4, false, Some(t_bound))
    ...
    pub fn check(&self) -> () {
        ...
        assert_eq!(!self.newton.initial_guess.is_empty(), true, "guess is empty");
        ...
        assert_eq!(self.newton.max_iterations>=1, true, "max_iterations is empty");
        assert_eq!(self.newton.dt>=0.0, true, "h is empty");
        assert_eq!(self.global_timestepping==true||self.h.is_none(), true, "for global timestepping h must be set");
    ...
    //  println!("guess = {:?}, t_i = {:?} ", &guess_i, t_i);
        ...
        if result.is_none(){
            ...
            return (false, Some("maximum number of iterations reached".to_string()));
        } else {
...
///________________________________________________________________________________________________________________________________
      ...
      if t == self.t_bound{
        ...
        let (success, message_) = self._step_impl() ;
          ...
          if success == false{
              ...
              if  (self.t - self.t_bound) >= 0.0{ //self.Solver_instance.direction *
                  self.status = "finished".to_string();
              }
          }
    ...
    pub fn plot_result(&self) -> (){
        plots(self.newton.arg.clone(), self.newton.values.clone(), self.t_result.clone(), self.y_result.clone());
        println!("result plotted");
    }

    pub fn get_result(&self) -> (Option<DVector<f64>>, Option<DMatrix<f64>>) {
        ( Some(self.t_result.clone()),
    
        Some(self.y_result.clone())
    )
}
