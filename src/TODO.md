...
Jacobian для Numerical path, возможный Frozen Newton route для дорогих Jacobian
и более богатые story tests уровня BVP_Damp.
...
P1.4b	API design	Positional solve_bvp API остается неудобным	BVP_sci_faer.rs / BVP_sci_numerical.rs	1-2 дня
	Промежуточно закрыто: добавлен closure-based `NumericalBvpClosureProblem` с `new_fd()` и
	`new_with_jacobian()` для чистого numerical path без символики; покрыты FD,
	analytical и parameterized closure-cases. Positional solve_bvp как legacy-style
	интерфейс всё ещё остаётся отдельным хвостом.
...
P1.4b остается открытым: дублирование `solve_bvp()` / `solve_bvp_sparse()`
закрыто, но публичный низкоуровневый вход все еще задается длинным позиционным
списком аргументов. Нужно сделать builder/options route по аналогии с
`BvpSciSolverOptions` / `BVPwrap::new_with_options` и numerical API, оставив
старые функции тонкими backward-compatible wrappers. Важно: это не builder
method explosion из P2.9, а отдельная задача убрать "скормить 12 параметров
функции" из нормального пользовательского пути.
...
всё еще стоит сближать с hardened BVP_Damp/LSODE2 lifecycle API. Дополнительно
...
P3.15	[x] Missing feature	Banded backend	—	основной parameter-free route закрыт
    ...
    point. Remaining limitation: unknown-parameter (`k > 0`) direct bordered
    assembly still uses the established Sparse compatibility route.
