//! Руководство по BVP_sci через Lambdify.
//!
//! Этот пример показывает symbolic baseline route для `BVP_sci`:
//! `ExprLegacy + Lambdify + sparse runtime planning`.
//!
//! Это самый простой symbolic entry point, когда у вас уже есть уравнения в
//! виде `Expr`, но внешний артефакт компилировать не хочется.
//!
//! Запуск:
//! `cargo run --example rus_bvp_sci_lambdify_guide`
...
fn main() {
    println!("Руководство по BVP_sci через Lambdify");
    println!("=====================================");
    println!("Это symbolic baseline route.");
    println!("Используйте его, когда нужен самый чистый one-shot symbolic workflow.");
    println!();
