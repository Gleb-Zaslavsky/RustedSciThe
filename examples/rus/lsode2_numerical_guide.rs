//! Гайд по чисто числовому пути LSODE2
//!
//! Цель:
//! показать "pure numerical" путь, где пользовательский код передаёт
//! callbacks напрямую, без символических уравнений, Lambdify и AOT.
//!
//! Мы решаем одну и ту же IVP-задачу в двух режимах:
//! 1. аналитическая невязка + аналитический якобиан
//! 2. аналитическая невязка + якобиан по конечным разностям
//!
//! Задача:
//! y' = -2y, y(0)=1, t in [0,1], exact y(1)=exp(-2) ~= 0.13533528
//!
//! Почему этот гайд важен:
//! - это минимальный интеграционный путь для тех, у кого формулы уже есть в Rust-коде;
//! - публичный API на этом пути не требует символических `Expr`-заглушек;
//! - он показывает, за что отвечает каждый knob конфигурации LSODE2;
//! - он даёт базовую точку сравнения перед переходом к symbolic Lambdify/AOT.
//!
//! запуск: cargo run --example lsode2_numerical_guide
//!
