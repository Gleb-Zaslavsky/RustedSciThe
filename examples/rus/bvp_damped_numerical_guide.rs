//! Гайд по чисто числовому BVP Damped
//!
//! Цель:
//! показать чисто числовой путь, где обычные Rust closures являются источником
//! истины и в солвер не передаётся никакой символический placeholder.
//!
//! Мы решаем одну и ту же BVP в двух режимах:
//! 1. residual closure + якобиан по конечным разностям
//! 2. residual closure + аналитический непрерывный якобиан `df/dy`
//!
//! Задача:
//!
//! ```text
...
//! - этот путь подходит, когда модель уже существует как Rust-код;
//! - числовые конструкторы не требуют бессмысленных символических заглушек;
//! - для дискретизированной BVP-матрицы по-прежнему доступна banded-линейная алгебра.
//! запуск: cargo run --example bvp_damped_numerical_guide
...
fn main() {
    ...
    // Путь 1: задаём только непрерывную RHS. Большой Newton- якобиан
    // дискретизированной BVP получается методом конечных разностей.
    ...
    // Путь 2: задаём маленький узловой якобиан. RustedSciThe собирает
    // его дискретизированный глобальный banded-эквивалент внутри.
