//! Руководство по LM fitting.
//!
//! Здесь показан классический путь symbolic least-squares:
//! - задаём модель через символические выражения,
//! - подаём данные x/y,
//! - указываем неизвестные параметры и начальное приближение,
//! - вызываем `build()`, после чего строятся якобиан и решение LM.
//!
...
fn main() {
    println!("=== Руководство по LM Fitting ===\n");
    linear_string_model();
    quadratic_native_model();
}

fn linear_string_model() {
    println!("Пример 1: линейная модель из строки");
    println!("Модель: y = a * x + b");
    println!("Данные:  y = 3x + 2\n");
...
fn quadratic_native_model() {
    println!("Пример 2: квадратичная модель через native Expr");
    println!("Модель: y = a * x^2 + b * x + c");
    println!("Данные:  y = 2x^2 + 3x + 1\n");
